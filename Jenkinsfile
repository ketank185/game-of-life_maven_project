	...
	stages {
		...
		stage ("downloading apache tomcat and deploying war in webapps") {
			steps {
				...
				sh 'ansible all -bm command -a "sh /home/mayu/apache-tomcat-9.0.73/bin/shutdown.sh remote_src=yes"'
				sh 'ansible all -bm command -a "sh /home/mayu/apache-tomcat-9.0.73/bin/startup.sh remote_src=yes"'
				}
		}
	}	
}
